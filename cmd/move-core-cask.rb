...
module Homebrew
  module Cmd
    class MoveCoreCask < AbstractCommand
      DEFAULT_HIDDEN_TAP_LOCATION = T.let((HOMEBREW_PREFIX/".hidden-taps").freeze, Pathname)
      HOMEBREW_TAP_LIBRARY = T.let((HOMEBREW_LIBRARY/"Taps/homebrew").freeze, Pathname)
      ...
      sig { override.void }
      def run
        hidden_tap_location = Pathname(args.location || DEFAULT_HIDDEN_TAP_LOCATION)
        ...
        source_dir, target_dir, action_str = if args.tap? || (!args.untap? && taps.none?(&:installed?))
